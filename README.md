...
- [castrocrest/mcp-probe-cli](https://github.com/castrocrest/mcp-probe-cli) 🐍 - Headless MCP server conformance tester, zero dependencies (`pip install mcp-conform`). Catches spec violations before push: JSON-RPC envelope, protocol version, tool schema validity (including `true`/`false` boolean schemas rejected by Claude Code), and method-not-found compliance. CI-composable via exit codes.
